...
The Projects application has one internal history: index → project summary → full case → evidence child. Summary and full case replace one another inside the same Projects window; they never create another taskbar item. Back restores the selected project and useful scroll position. A directly loaded case route uses the Projects-owned route host while preserving the same chapter and evidence behavior.
